...
export default function OrderTracker({ orderId, subtitle, backTo, backLabel }) {
  ...
  /* Opened by a successful receipt, not by a tap. */
  ...
  const receive = async () => {
    ...
    /* Ask now, while they still have the meal in mind and the screen in
       front of them. */
    setRating(true);
  };

  const rate = async (stars, text) => {
    const out = await shop.rateOrder(order.id, stars, text);
    setRating(false);
    if (!out.ok) return alert.error(errorText(out.error, t, n, out));
    alert.success(t('Thank you — your rating helps the next customer.'));
  };
 ...
 */
function RateSheet({ open, cook, onClose, onSubmit }) {
  const { colors, shadow } = useTheme();
  const { t } = useLang();
    ...
    <Modal visible={open} transparent animationType="fade" onRequestClose={onClose}>
      ...
      >
        ...
        >
          ...
          <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'center', paddingVertical: 4 }}>
            {[1, 2, 3, 4, 5].map((value) => (
              <Pressable
                key={value}
